#pragma once
// ============================================================================
// Optimized MemDumper — Gap-tolerantマージ + memcpy最適化版
// ============================================================================
...
// 最適化版メモリダンパー
// - Gap-tolerant merge: 64B以内のGapは吸収してmemcpy回数を削減
...
public:
    ...
    size_t GetTotalSize() const { return _totalCopySize; }
    size_t GetEntryCount() const { return _mergedEntries.size(); }
    size_t GetOriginalEntryCount() const { return _mergedEntries.size(); }

    // memcpy ベース（コンパイラが最適化する最速パス）
    void SaveState(char* dest) const {
        for (size_t i = 0; i < _mergedEntries.size(); ++i) {
            memcpy(dest + _offsets[i],
                   reinterpret_cast<const void*>(_mergedEntries[i].addr),
    ...
    void LoadState(const char* src) const {
        for (size_t i = 0; i < _mergedEntries.size(); ++i) {
            memcpy(reinterpret_cast<void*>(_mergedEntries[i].addr),
                   src + _offsets[i],
                   _mergedEntries[i].size);
        }
    }
...
private:
    ...
    std::vector<size_t>    _offsets;  // 事前計算済みオフセット
