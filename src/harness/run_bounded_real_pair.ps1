...
$taskTest=Join-Path $taskRoot '_TEST_MBAACC'
if($TestRoot){$taskTest=(Resolve-Path -LiteralPath $TestRoot).Path}
$taskOut=Join-Path $taskRoot ('build_logs\bounded_real_'+(Get-Date -Format 'yyyyMMdd_HHmmss'))
...
try {
    foreach($taskSide in 1,2) {
        ...
        foreach($taskFile in 'CCCaster_v10.exe','libcccaster_hook.dll') {
    ...
    foreach($taskSide in 1,2) {
        ...
        $taskLaunchers+=Start-Process (Join-Path $taskDir 'CCCaster_v10.exe') -WindowStyle Hidden -PassThru -WorkingDirectory $taskDir -ArgumentList $taskArgs -RedirectStandardOutput (Join-Path $taskOut "launcher_$taskSide.log") -RedirectStandardError (Join-Path $taskOut "launcher_$taskSide.err")
