...
void Playback::Pace(bool fast, bool visible) {
    ...
    if (fast) {
        ...
        if (visible) core::SpeedFlags::RenderSkip().store(false, std::memory_order_release);
        return;
    }
...
bool Playback::Step(Phase phase) {
    ...
    const bool showTransition = phase == Phase::Loading || phase == Phase::CharaSelect ||
        (phase == Phase::InGame && mem.IntroState() != 0);
    if (showTransition) core::SpeedFlags::RenderSkip().store(false, std::memory_order_release);
    ...
    if (!have_) { Control::WriteInput({}, {}); Pace(phase == Phase::Loading, showTransition); return true; }
    for (unsigned events = 0; events < 8; ++events) {
        if (pending_.kind == Start || pending_.kind == Selection) {
            ...
            if (phase == Phase::CharaSelect) {
                ...
                Control::WriteInput(p1, p2); Pace(true, true); return true;
            }
            if (phase != Phase::InGame) { Control::WriteInput({}, {}); Pace(phase == Phase::Loading, showTransition); return true; }
            ...
            Pace(catchupPending_ || introFast, showTransition);
            return true;
        } else return false;
