...
$taskOut=Join-Path $taskRoot ('test/logs/spectator_'+(Get-Date -Format 'yyyyMMdd_HHmmss'))
...
try {
    ...
    $taskDir=Join-Path $taskTest 'MBAACC_3/cccaster_B'
    foreach($taskFile in 'CCCaster_B.exe','libcccaster_hook.dll','CCCaster_B_GUI.exe') {
    ...
    if($UseCode) {
        ...
        $taskCode=[regex]::Match($taskHostOutput,'\[SPECTATOR CODE\] ((?:S-)?[A-Za-z0-9]+)').Groups[1].Value
        if(!$taskCode){throw 'ホストの観戦コードなし'}
        $taskViewerArgs="--spectate --hash $taskCode"
    }
    $taskViewer=Start-Process (Join-Path $taskDir 'CCCaster_B.exe') -WindowStyle Hidden -PassThru -WorkingDirectory $taskDir -ArgumentList $taskViewerArgs -RedirectStandardOutput (Join-Path $taskOut 'viewer_launcher.log') -RedirectStandardError (Join-Path $taskOut 'viewer_launcher.err')
