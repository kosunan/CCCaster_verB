...
class ConnectionHash {
  public:
    ...
    /// 観戦モード専用。通常コードと旧S-/s-接頭辞を受け付ける。
    static bool DecodeSpectator(const std::string &code, DecodedAddress &out) {
        const bool prefixed = code.rfind("S-", 0) == 0 || code.rfind("s-", 0) == 0;
        return Decode(prefixed ? code.substr(2) : code, out);
    }
